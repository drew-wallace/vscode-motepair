...
(function () {
	var vscode = require('vscode'),
		...
		// This is necessary until TextDocumentChangeEvent get's a source argument, defining where the edit came from (user or remote user)
		EDIT_PREVENTION_STRING = "THIS EDIT IS FROM THE SERVER: superawesomeamazingstringthatnoonewouldeverpaste97862193409#$%^&%#$%^6283971@#$%#$^*$%^&$%/.,.'];[]ASDFASDFASDFASDfasdfasdFASdSDFGDFTHEWRGeqrverv";
	...
	EventHandler = (function () {
		...
		EventHandler.prototype.getEditor = function (data) {
			...
			vscode.window.visibleTextEditors.forEach(function(editor) {
				var ref1;
				if (editor.document.uri.fsPath && editor.document.uri.fsPath.indexOf(file) !== -1) {
		...
		EventHandler.prototype.oninsert = function (data) {
			...
			editor.edit(function (textEditorEdit) {
				// This is necessary until TextDocumentChangeEvent get's a source argument, defining where the edit came from (user or remote user)
				textEditorEdit.insert(start, EDIT_PREVENTION_STRING);
				textEditorEdit.insert(start, data.text);
		...
		EventHandler.prototype.listen = function () {
			...
			this.subscriptions.push(vscode.workspace.onDidChangeTextDocument((function (_this) {
				return function (TextDocumentChangeEvent) {
					// This is necessary until TextDocumentChangeEvent get's a source argument, defining where the edit came from (user or remote user)
					if(TextDocumentChangeEvent.contentChanges[0].text && TextDocumentChangeEvent.contentChanges[0].text === EDIT_PREVENTION_STRING) {
						return;
					}
