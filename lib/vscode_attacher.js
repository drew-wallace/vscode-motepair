  ...
  function shareVSCodeEditor(TextEditor, ctx) {
    ...
    function applyToShareJS(TextEditor, TextDocumentChangeEvent) {
      ...
      if(TextDocumentChangeEvent.contentChanges[0].text.length === 0){
        ctx.remove(TextDocumentChangeEvent.document.offsetAt(TextDocumentChangeEvent.contentChanges[0].range.start), TextDocumentChangeEvent.document.offsetAt(TextDocumentChangeEvent.contentChanges[0].range.end));
      }

      if (TextDocumentChangeEvent.contentChanges[0].text.length > 0) {
        ctx.insert(TextDocumentChangeEvent.document.offsetAt(TextDocumentChangeEvent.contentChanges[0].range.start), TextDocumentChangeEvent.contentChanges[0].text);
      }
